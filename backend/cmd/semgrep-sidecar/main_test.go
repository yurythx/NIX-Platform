...
// Estes testes rodam o binário `semgrep` de verdade (pulados se não
// estiver no PATH) — mesmo princípio de rigor de cmd/trivy-sidecar:
// verificar contra a ferramenta real, não só contra um mock da resposta
// dela.
func skipIfNoSemgrep(t *testing.T) {
	t.Helper()
	if _, err := exec.LookPath("semgrep"); err != nil {
		t.Skip("semgrep not found in PATH; skipping sidecar test that shells out to the real binary")
	}
}
...
func TestHandleScan_RealSemgrepAgainstEmptyDir_Returns200WithJSON(t *testing.T) {
	skipIfNoSemgrep(t)
...
// Config vazio no corpo da requisição cai pro default embutido
// (defaultConfig) em vez de passar "--config ”" pro semgrep, que
// falharia — a única diferença real de contrato entre este sidecar e
// trivy-sidecar/gitleaks-sidecar (ver comentário no topo do arquivo).
func TestHandleScan_EmptyConfig_FallsBackToDefault(t *testing.T) {
	skipIfNoSemgrep(t)
	...
	rec := httptest.NewRecorder()
	body, _ := json.Marshal(scanRequest{Path: target, Config: ""})
	req := httptest.NewRequest(http.MethodPost, "/scan", bytes.NewReader(body))
	handleScan(testLogger())(rec, req)
