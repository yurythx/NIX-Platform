...
// Estes testes rodam o binário `gitleaks` de verdade (pulados se não
...
// dela.
func skipIfNoGitleaks(t *testing.T) {
	t.Helper()
	if _, err := exec.LookPath("gitleaks"); err != nil {
		t.Skip("gitleaks not found in PATH; skipping sidecar test that shells out to the real binary")
	}
}
...
func TestHandleScan_RealGitleaksAgainstEmptyDir_Returns200WithEmptyBody(t *testing.T) {
	skipIfNoGitleaks(t)
	...
	// gitleaks não escreve nada em stdout quando não acha nenhum segredo
	// (--report-path /dev/stdout com zero achados fica vazio) — um
	// diretório vazio precisa devolver 200 com corpo vazio, não um erro
	// de parsing.
	if rec.Body.Len() != 0 {
		t.Errorf("body = %q, want empty for a clean scan (gitleaks writes nothing when no secrets are found)", rec.Body.String())
	}
}
